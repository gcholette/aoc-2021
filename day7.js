...
const getMax = (positions) => positions.reduce((acc,curr) => curr > acc ? curr : acc, 0)
const getMin = (positions) => positions.reduce((acc,curr,i) => curr < acc[0] ? [curr, i] : acc, [0, 0])
...
function align(positions, variant = 1) {
 ...
 for (let tries = 0; tries <= max; tries++) {
    const newSolutions = positions.map(pos => {
      ...
      const wow = [...new Array(distance)].map((x, i) => i+1).reduce((acc,curr) => curr + acc, 0)
      return wow
    })
...
function challenge1() {
  ...
  const array = getChallengeData(day, true)[0].split(',').map(x => +x)
...
function challenge2() {
  ...
  const array = getChallengeData(day, false)[0].split(',').map(x => +x)
